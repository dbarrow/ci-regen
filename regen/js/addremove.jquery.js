...
(function ($) {
    $.fn.EnableMultiField = function (options) {
        options = $.extend({
            linkText: '<i class="icon-plus icon-white"></i>',
        ...
        return this.each(function () {
            ...
            $(self).append(" <a href='#' style='margin-top:5px;' class='btn btn-success " + options.linkClass + "'>" + options.linkText + "</a>");
                ...
                if (options.maxItemsAllowedToAdd === null || totalCount < options.maxItemsAllowedToAdd) {
                    ...
                    if (options.enableRemove && $(self).attr("uniqueId") != $(elem).parent().attr("uniqueId")) {
                        if ($(elem).parent().find("." + options.removeLinkClass).length === 0) {
                            $(elem).parent().append(" <a href='#' style='margin-top:5px;'class='btn btn-success" + options.removeLinkClass + "'>" + options.removeLinkText + "</a><br/>");
                    ...
                    if (options.enableRemove) {
                        if ($(newElem).find("." + options.removeLinkClass).length === 0) {
                            $(newElem).append(" <a href='#' style='margin-top:5px;' class='btn btn-danger " + options.removeLinkClass + "'>" + options.removeLinkText + "</a>");
                        }
                    ...
                    $(newElem).append(" <a href='#' style='margin-top:5px;' class='btn btn-success " + options.linkClass + "'>" + options.linkText + "</a>");
                    ...
                    if ($(elem).parent().find("." + options.linkClass).length > 0) {
                        if (options.enableRemove && $(self).attr("uniqueId") != $(prevParent).attr("uniqueId")) {
                            if ($(prevParent).find("." + options.removeLinkClass).length === 0) {
                                $(prevParent).append(" <a href='#' class='btn " + options.removeLinkClass + "'>" + options.removeLinkText + "</a>");
                            }
                        ...
                        $(prevParent).append(" <a href='#' style='margin-top:5px;' class='btn btn-success " + options.linkClass + "'>" + options.linkText + "</a>");
